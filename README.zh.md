...
- 运行间漂移:当某个 star 区间超过 1000 条查询窗口时,`--bands` 只能取该区间的 1000 个成员,而同分并列的返回顺序不保证稳定,star 数也会在两次运行间变化。本仓库连续两次运行的仓库数是 5004 和 5007,not-a-plugin 是 230 和 259。所以带星段的索引适合当高 star 噪声区的**过滤器**,不是普查结果;需要精确成员时用 `--full`。
